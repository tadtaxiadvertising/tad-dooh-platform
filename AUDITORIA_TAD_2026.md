...
- **Optimización de Build (SRE)**: Dockerfile de Backend migrado a `npm ci` con limpieza agresiva de capas. Configurado `max-old-space-size=850` para aprovechar la expansión a 1GB RAM en el VPS, asegurando despliegues exitosos y mayor performance en tiempo de ejecución. (02/Abr/2026)
- **Aceptación Digital de Acuerdos**: Implementada "Firma Digital" obligatoria para (1) Acuerdo de Responsabilidad y Comodato del equipo y (2) Contrato de Prestación de Servicios independiente, alineado con el Código de Trabajo de RD (Art. 2) para evitar laboralización. Se eliminaron menciones a seguros médicos para mitigar riesgos legales de cobertura. (03/Abr/2026)
- **Corrección de Rutas Portal de Choferes**: Se detectaron y corrigieron errores 404 en el portal. Se actualizó la variable `API` para incluir el prefijo `/api/v1` exigido por el backend de NestJS. También se estabilizó el cargador de Sentry. (03/Abr/2026)
- **Resiliencia de Componentes UI**: Hardened `AntigravityButton` con lazy-loading de Supabase para evitar fallos de inicialización en el bundle de producción. (02/Abr/2026)
